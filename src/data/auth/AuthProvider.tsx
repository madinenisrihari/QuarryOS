...
/** Roles come from `app_metadata` (only writable server-side). Unknown means least privilege. */
function toSessionUser(u: { id: string; email?: string | null; user_metadata?: Record<string, unknown>; app_metadata?: Record<string, unknown> }): SessionUser {
  const r = u.app_metadata?.role as Role | undefined
  return {
    id: u.id,
    email: u.email ?? '',
    name: (u.user_metadata?.full_name as string | undefined) ?? (u.email ?? 'User').split('@')[0]!,
    role: r && ROLES.includes(r) ? r : 'worker',
    organisation: (u.user_metadata?.company as string | undefined) ?? 'Your organisation',
    demo: false,
  }
}
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (supabase) {
      supabase.auth.getSession().then(({ data }) => {
        setUser(data.session ? toSessionUser(data.session.user) : null)
        setLoading(false)
      })
      const { data: sub } = supabase.auth.onAuthStateChange((_e, session) => setUser(session ? toSessionUser(session.user) : null))
      return () => sub.subscription.unsubscribe()
    }
    try {
      const raw = localStorage.getItem(DEMO_KEY)
      if (raw) setUser({ ...(DEMO_USERS[JSON.parse(raw) as Role] ?? DEMO_USERS.owner), demo: true })
    } catch { /* storage unavailable: stay signed out */ }
    setLoading(false)
  }, [])

  const signInDemo = useCallback((role: Role) => {
    try { localStorage.setItem(DEMO_KEY, JSON.stringify(role)) } catch { /* ignore */ }
    setUser({ ...DEMO_USERS[role], demo: true })
  }, [])

  const signIn = useCallback(async (email: string, password: string) => {
    if (!supabase) throw new Error('Supabase is not configured. Use the demo sign-in below.')
    const { error } = await supabase.auth.signInWithPassword({ email, password })
    if (error) throw new Error(error.message)
  }, [])

  const signUp = useCallback(async ({ name, email, password, company }: { name: string; email: string; password: string; company: string }) => {
    if (!supabase) throw new Error('Supabase is not configured. Use the demo sign-in instead.')
  ...
  const value = useMemo<AuthContextValue>(
    () => ({ user, loading, demoMode: !isSupabaseConfigured, signIn, signUp, signInDemo, signOut }),
